...
def rnn_backward(dh, cache):
    ...
    for t in range(T):
        cache_step = cache[T-1-t]
        dh_step = dh[:,T-1-t,:] + dprev_h
        dx[:,T-1-t,:], dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dh_step, cache_step)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    len_A = int(A.shape[1]/4)
    ii = 1 / (1 + np.exp(-A[:,range(len_A)]))
    ff = 1 / (1 + np.exp(-A[:,range(len_A, len_A*2)]))
    oo = 1 / (1 + np.exp(-A[:,range(len_A*2, len_A*3)]))
    gg = np.tanh(A[:,range(len_A*3, len_A*4)])
    next_c = np.multiply(ff,prev_c) + np.multiply(ii,gg)
    next_h = np.multiply(oo,np.tanh(next_c))
    
    cache = (x, prev_h, prev_c, Wx, Wh, b, next_h, next_c, ii, ff, oo, gg)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    x, prev_h, prev_c, Wx, Wh, b, next_h, next_c, ii, ff, oo, gg = cache
    ...
    da_i_partial = dtanh_dc * gg
    da_g_partial = dtanh_dc * ii
    da_f_partial = dtanh_dc * prev_c

    # Backprop through gate activation functions to calculate gate derivatives.
    da_i = ii*(1-ii) * da_i_partial
    da_f = ff*(1-ff) * da_f_partial
    da_o = oo*(1-oo) * da_o_partial
    da_g = (1-np.square(gg)) * da_g_partial
    ...
    dx = np.matmul(da_vector, Wx.T)
    dWx = np.matmul(x.T, da_vector)
    dprev_h = np.matmul(da_vector, Wh.T)
    dWh = np.matmul(prev_h.T, da_vector)
...
def lstm_backward(dh, cache):
    ...
    x, prev_h, prev_c, Wx, Wh, b, next_h, next_c, ii, ff, oo, gg = cache[0]
    N, T, H = dh.shape
    D, _ = Wx.shape
    ...
    for t in range(T):
        ...
        dx[:,T-1-t,:], dprev_h, dprev_c, dWx_t, dWh_t, db_t = lstm_step_backward(dh_step, dprev_c, cache_step)
